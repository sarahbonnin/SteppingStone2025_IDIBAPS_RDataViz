...
Let's import data from: **DataViz_source_files-main/files/stats_continents_barcelona_2013-2023_long.csv** 
...
If we set **stat="identity"** in geom_bar(), R uses the **sum of the variable** specified in **y=**, **grouped by the x variable**.

In the following example, we plot the sum of foreign residents in Barcelona (Population provided in **y**) per year (Year provided in **x**):
...
ggplot2 provides **facet** functions in R, that are used to **split the plot**, according to a given variable.
